...
EXCEL_COLUMNS = [
    "time", "file", "commit_hash", "config_hash", "hash_file", "weight_hash", "weight",
...
def test_get_results_writes_empty_solution_artifacts_and_returns_none(tmp_path):
    ...
    config_df = pd.read_excel(
        excel_path, sheet_name="run_configs", engine="openpyxl"
    )
    parquet_df = pd.read_parquet(parquet_path)
    assert list(excel_df.columns) == EXCEL_COLUMNS
    assert excel_df.empty
    assert config_df.to_dict("records") == [
        {
            "config_hash": _data()["config_hash"],
            "config_json": _data()["config_json"],
        }
    ]
...
def test_get_results_preserves_excel_and_parquet_contract(tmp_path):
    ...
    assert excel_df.loc[0, "config_hash"] == data["config_hash"]
    assert config_df.to_dict("records") == [
        {
