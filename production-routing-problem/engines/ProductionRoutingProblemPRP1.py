...
class ProductionRoutingProblemPRP1:
    ...
    def createDecisionVariables(self):
        ...
        for i in range(self.CONST_CUSTOMER_I):
            for t in range(self.CONST_PERIOD_T):
                ...
                if(i==0):
                    self.z[i,t] = self.model.addVar(vtype=GRB.INTEGER, name=f"z[{i},{t}]") 
                else:
    ...
    def createRestrictionBalanceStockFlowFactoryAndCustomers(self):
        ...
        for t in range(1,self.CONST_PERIOD_T):
            ...
            self.model.addConstr(  self.I[0, t-1] + self.p[t] == r2+self.I[0,t], name=f"rest_2_{t}_{i}")
    ...
    def createRestrictionVehicleLoadingAndSubTourElimination(self):  
        for i in range(self.CONST_CUSTOMER_I):
            for j in range(self.CONST_CUSTOMER_J):
                for t in range(self.CONST_PERIOD_T):
                    self.model.addConstr(self.w[i,t] - self.w[j,t] >= self.q[i,t] - self.MT[i][t]*(1 - self.x[i,j,t]), name=f"rest_11_{t}")

        for t in range(self.CONST_PERIOD_T):
            for i in range(self.CONST_CUSTOMER_I):
                self.model.addConstr(self.w[i,t] <= self.data.Q*self.z[i,t], name=f"rest_12_{t}")
    ...
    def outModel(self):
        ...
        self.model.write("./out/model/modelo_period_"+str(self.data.CONST_PERIOD_T)+"_clients_"+str(self.CONST_CUSTOMER_I)+".lp")
