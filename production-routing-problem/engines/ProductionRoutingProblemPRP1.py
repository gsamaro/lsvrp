...
class ProductionRoutingProblemPRP1:
    ...
    def createDecisionVariables(self):
        ...
        for i in range(self.CONST_CUSTOMER_I):
            for j in range(self.CONST_CUSTOMER_J):
                for t in range(self.CONST_PERIOD_T):
                    self.x[i, j, t] = self.model.addVar(
        ...
        for i in range(self.CONST_CUSTOMER_I):
            for t in range(self.CONST_PERIOD_T):
                ...
                if i == 0:
                    self.z[i, t] = self.model.addVar(
                        vtype=GRB.INTEGER, name=f"z[{i},{t}]"
                    )
                else:
                    self.z[i, t] = self.model.addVar(
                        vtype=GRB.BINARY, name=f"z[{i},{t}]"
                    )
    ...
    def crateObjectiveFunction(self):
        ...
        for t in range(self.CONST_PERIOD_T):
            ...
            objExpr += (
                self.data.u * self.p[t]
                + self.data.f * self.y[t]
                + objExpr_1
                + objExpr_2
            )
    ...
    def createRestrictionBalanceStockFlowFactoryAndCustomers(self):
        ...
        for t in range(1, self.CONST_PERIOD_T):
            r2 = 0
            for i in range(1, self.CONST_CUSTOMER_I):
                r2 += self.q[i, t]
                self.model.addConstr(
                    self.I[i, t - 1] + self.q[i, t] == self.data.d[i][t] + self.I[i, t],
                    name=f"rest_3_{t}_{i}",
                )
            self.model.addConstr(
                self.I[0, t - 1] + self.p[t] == r2 + self.I[0, t],
                name=f"rest_2_{t}_{i}",
            )
    ...
    def createRestrictionLimitMaximumInventory(self):
        ...
        for t in range(1, self.CONST_PERIOD_T):
            for i in range(1, self.CONST_CUSTOMER_I):
                self.model.addConstr(
                    self.I[i, t - 1] + self.q[i, t] <= self.data.L[i],
                    name=f"rest_6_{t}",
                )
    ...
    def createRestrictionAllowPositiveDeliveryQuantityOnly(self):
        for t in range(self.CONST_PERIOD_T):
            for i in range(1, self.CONST_CUSTOMER_I):
                self.model.addConstr(
                    self.q[i, t] <= self.MT[i][t] * self.z[i, t], name=f"rest_7_{t}_{i}"
                )
    ...
    def createRestrictionEachCustomerCanBeVistitedByMostOneVehicle(self):
        for t in range(self.CONST_PERIOD_T):
            for i in range(1, self.CONST_CUSTOMER_I):
                r8 = 0
                for j in range(1, self.CONST_CUSTOMER_J):
                    r8 += self.x[i, j, t]
    ...
    def createRestrictionVehicleFlowConservation(self):
        for t in range(self.CONST_PERIOD_T):
            for i in range(self.CONST_CUSTOMER_I):
                ...
                self.model.addConstr(
                    r9_1 + r9_2 == 2 * self.z[i, t], name=f"rest_9_{t}_{i}"
                )
    ...
    def createRestrictionVehicleLoadingAndSubTourElimination(self):
        for i in range(self.CONST_CUSTOMER_I):
            for j in range(self.CONST_CUSTOMER_J):
                for t in range(self.CONST_PERIOD_T):
                    self.model.addConstr(
                        self.w[i, t] - self.w[j, t]
                        >= self.q[i, t] - self.MT[i][t] * (1 - self.x[i, j, t]),
                        name=f"rest_11_{t}",
                    )
        ...
        for t in range(self.CONST_PERIOD_T):
            for i in range(self.CONST_CUSTOMER_I):
                self.model.addConstr(
                    self.w[i, t] <= self.data.Q * self.z[i, t], name=f"rest_12_{t}"
                )
    ...
    def createM(self):
        for t in range(self.CONST_PERIOD_T):
            sum_d = 0
            for i in range(self.CONST_CUSTOMER_I):
                sum_d = sum_d + self.data.d[i][t]
            self.M.append(min(self.data.C, sum_d))
        ...
        for i in range(self.CONST_CUSTOMER_I):
            ...
            for t in range(self.CONST_PERIOD_T):
                sum_d = sum_d + self.data.d[i][t]
                mt.append(min(self.data.L[i], self.data.Q, sum_d))
            self.MT.append(mt)

    def outModel(self):
        self.model.Params.OutputFlag = 1
        self.model.write(
            "./out/model/modelo_period_"
            + str(self.data.CONST_PERIOD_T)
            + "_clients_"
            + str(self.CONST_CUSTOMER_I)
            + ".lp"
        )
