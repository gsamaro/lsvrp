...
from src.solvers.GreedyRandomizedConstructionRoute import (
    GreedyRandomizedConstructionRoute as GR,
)
...
class MultProductProdctionRoutingProblemGreedyConstructiveHeuristic:
    ...
    def construirSolucao(self):
        ...
        estoque_i_p = [
            [self.I_p_i_0[p][i] for p in range(self.p)] for i in range(self.i)
        ]
        ...
        for t in range(self.t):
                    ...
                    if estoque_i_p[i][p] < demanda_t:
                        ...
                        if capacidade_veiculo_totais[veiculo_corrente] < 0:
                            ...
                            if veiculo_corrente > self.v:
                                self.log.error(
                                    "modelo Inviavel pelas restrições de capacidade do veiculo"
                                )

                        if capacidade_producao_restante < 0:
                            self.log.error(
                                "modelo Inviavel pelas restrições de produção"
                            )
                            break
            ...
            while (
                ...
                if (
                    capacidade_producao_restante < 0
                    or capacidade_veiculo_totais[veiculo_corrente] < 0
                ):
                    self.log.warning(
                        f"Produção está negátiva: {capacidade_producao_restante} ou capcidade_veiculo negativo:{capacidade_veiculo_totais[veiculo_corrente]}"
                    )
                    capacidade_producao_restante += sum(total_p)
                    capacidade_veiculo_totais[veiculo_corrente] += sum(total_p)
                    iter += 1
                    ...
                    for p in range(len(total_p)):
                        ...
                        candidatos = [
                            c for c in candidatos if not (c[0] == i and c[1] == p)
                        ]
            ...
            for i, linha in enumerate(solucao_t_i_p[t]):
                ...
                if any(q > 0 for q in prod):  # se tem algo produzido para o cliente
                    ...
                    dem_t.append(
                        prod
                    )  # agora vai com vetor por produto, não o somatório
            ...
            route, distance, demandas = self.greedyRoute.greedyRandomizedConstruction(
                candidates_t,
                dem_t,
                capacities,
                D,
        ...
        self.solution = {"production": solucao_t_i_p, "routes": routes}
    ...
    def convertVariables(self):
        ...
        for t in range(len(final_solution["routes"])):
            for v in range(len(final_solution["routes"][t]["demandas"])):
                for i in range(
                    len(final_solution["routes"][t]["demandas"][v]["entregas"])
                ):
                    origem_i = final_solution["routes"][t]["demandas"][v]["entregas"][
                        i
                    ]["cliente"]
                    if i + 1 == len(final_solution["routes"][t]["route"][v]):
                        destino_j = 0
                    else:
                        destino_j = final_solution["routes"][t]["demandas"][v][
                            "entregas"
                        ][i + 1]["cliente"]
                    for p in range(
                        len(
                            final_solution["routes"][t]["demandas"][v]["entregas"][i][
                                "produtos"
                            ]
                        )
                    ):
                        R[p, v, origem_i, destino_j, t] = final_solution["routes"][t][
                            "demandas"
                        ][v]["entregas"][i]["produtos"][p]["restante_veiculo"]
                for i in range(
                    len(final_solution["routes"][t]["demandas"][v]["entregas"])
                ):
                    origem_i = final_solution["routes"][t]["demandas"][v]["entregas"][
                    ...
                    for p in range(
                        ...
                        Q[p, v, origem_i, t] = final_solution["routes"][t]["demandas"][
                            v
                        ]["entregas"][i]["produtos"][p]["qte_entregue"]
        ...
        for t in range(len(final_solution["production"])):
            ...
            for i in range(len(final_solution["production"][t])):
                for p in range(len(final_solution["production"][t][i])):
                    ...
                    I[p, i, t] = (
                        final_solution["production"][t][i][p]["estoque"]
                        - final_solution["production"][t][i][p]["demanda"]
                    )
    ...
    def solver(self, numThreads=None, timeLimit=None):
        ...
        if self.mitStart == True:
            self.solverGurobi = MPPRP(
                ...
                self.dir,
                self.log,
                {"start": True, "variables": self.variables},
            )
