    ...
    def process(datas=None):
        ...
        for data in datas:
            ...
            for file in data["files"]:
                ...
                with open(
                    f"./out/{directory}/{name_file}/result.json", "r", encoding="utf-8"
                ) as f:
            ...
            results.append({"dir": directory, "resultsFile": results_file})
        ...
        for result in results:
            ...
            for file_result in result["resultsFile"]:
                print(
                    f"{file_result['name']} - {file_result['class']} - {file_result['seed']}\n"
                )
                rows.append(
                    [
            ...
            df = df.sort_values(
                by=["Instância", "Seed", "Classe"], ascending=[True, True, True]
            )
            df = df.round(2)
            ...
            with open(
                f"out/{result['dir']}/{result['dir']}.txt", "w", encoding="utf-8"
            ) as f:
                f.write(latex)
